...
static inline uint secondsToTicks(const uint sec);
static inline uint getTimeIn(const uint sec);

// Graphics.h
static inline void setWindowMode(const WindowMode mode);
static inline void setWindowResizable(const bool resizable);
static inline bool getWindowResizable(void);
static inline void setWindowLen(const Length len);
static inline Length getWindowLen(void);
static inline bool windowResized(void);
static inline Length getWindowMid(void);
static inline bool coordInWindow(const Coord pos);
static inline Length maximizeWindow(void);
static inline Length restoreWindow(void);
static inline void setWindowSize(const uint x, const uint y);
static inline void setBlend(const BlendMode mode);
static inline void drawPixel(const uint x, const uint y);
static inline void drawPixelCoord(const Coord pos);
static inline void drawLine(const uint x1, const uint y1, const uint x2, const uint y2);
static inline void drawLineCoords(const Coord pos1, const Coord pos2);
static inline void drawLineCoordPair(const CoordPair pair);
static inline void drawHLine(const uint x, const uint y, int len);
static inline void drawVLine(const uint x, const uint y, int len);
static inline void drawRect(const uint x, const uint y, const uint xlen, const uint ylen);
static inline void drawRectCentered(const uint x, const uint y, const uint xlen, const uint ylen);
static inline void fillRect(const uint x, const uint y, const uint xlen, const uint ylen);
static inline void fillRectCentered(const uint x, const uint y, const uint xlen, const uint ylen);
static inline void drawRectCoords(const Coord pos1, const Coord pos2);
static inline void drawRectCenteredCoords(const Coord pos1, const Coord pos2);
static inline void fillRectCoords(const Coord pos1, const Coord pos2);
static inline void fillRectCenteredCoords(const Coord pos1, const Coord pos2);
static inline void drawRectCoordLength(const Coord pos, const Length len);
static inline void drawRectCenteredCoordLength(const Coord pos, const Length len);
static inline void fillRectCoordLength(const Coord pos, const Length len);
static inline void fillRectCenteredCoordLength(const Coord pos, const Length len);
static inline void drawRectRect(const Rect rect);
static inline void fillRectRect(const Rect rect);
static inline void drawSquareCoord(const Coord pos, const uint len);
static inline void drawSquare(const uint x, const uint y, const uint len);
static inline void fillSquare(const uint x, const uint y, const uint len);
static inline void fillSquareResize(const uint x, const uint y, const uint len, const int resize);
static inline void fillSquareCoord(const Coord pos, const uint len);
static inline void fillSquareCoordResize(const Coord pos, const uint len, const int resize);
static inline void fillBorder(uint x, uint y, uint xlen, uint ylen, int b);
static inline void fillBorderCoords(const Coord pos, const Length len, const int b);
static inline void fillBorderCoordSquare(const Coord pos, const uint len, const int b);
static inline void drawCircle(const uint x, const uint y, const uint radius);
static inline void fillCircle(const uint x, const uint y, const uint radius);
static inline void drawCircleCoord(const Coord pos, const uint radius);
static inline void fillCircleCoord(const Coord pos, const uint radius);
static inline Color getColor(void);
static inline void setColor(const Color c);
static inline u32 colorToU32(const Color c);
static inline Color u32ToColor(const u32 u);
static inline void drawTri(const Coord pos1, const Coord pos2, const Coord pos3);
static inline void fillTri(const Coord pos1, const Coord pos2, const Coord pos3);
static inline void setRGB(const u8 r, const u8 g, const u8 b);
static inline void setRGBA(const u8 r, const u8 g, const u8 b, const u8 a);
static inline void fillScreen(void);
...
// Text.h
static inline Img *textImg(const char *text);
static inline Texture *textTexture(const char *text);
static inline Length drawText(const char *text, const int x, const int y);
static inline Coord drawTextCoord(const char *text, const Coord pos);
static inline Length drawTextCentered(const char *text, const int x, const int y);
static inline Length drawTextCenteredCoord(const char *text, const Coord pos);
static inline Length getTextLength(const char *text);
static inline int getTextXLen(const char *text);
static inline int getTextYLen(const char *text);
...
// Image.h
static inline void freeImg(Img *image);
static inline void freeTexture(Texture *t);
static inline Img *loadImg(const char *imgFile);
static inline Texture *imgTexture(Img *img);
static inline Texture *loadTexture(const char *imgFile);
static inline void drawImg(Img *image);
static inline void loadDrawImg(const char *imgFile);
static inline Length textureLen(Texture *texture);
static inline Rect textureRect(Texture *texture, const Coord pos);
static inline Length drawTexture(Texture *texture, const int x, const int y);
static inline Length drawTextureCoord(Texture *texture, const Coord pos);
static inline Length drawTextureCentered(Texture *texture, const int x, const int y);
static inline Length drawTextureCenteredCoord(Texture *texture, const Coord pos);
static inline Length drawTextureResize(Texture *texture, const int x, const int y, const int xlen, const int ylen);
static inline Length drawTextureCoordResize(Texture *texture, const Coord pos, const Length len);
static inline Length drawTextureCenteredResize(Texture *texture, const int x, const int y, const int xlen, const int ylen);
static inline Length drawTextureCenteredCoordResize(Texture *texture, const Coord pos, const Length len);
void img_quit(void);
void img_init(void);
